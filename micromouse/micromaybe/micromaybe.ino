...
#define TRIGGER_PINL  2  // Arduino pin tied to trigger pin on ping sensor.
#define ECHO_PINL     8  // Arduino pin tied to echo pin on ping sensor.
...
#define TRIGGER_PINR  4  // Arduino pin tied to trigger pin on ping sensor.
#define ECHO_PINR     13  // Arduino pin tied to echo pin on ping sensor.
...
void pid_start() {
  ...
  if (RMS < 0) {
    ...
    RMS = map(RMS , 0 , -255, 50, 255);
    LMS = map(LMS , 0 , 255, 50, 255);
    analogWrite(enA , RMS);
    analogWrite(enB , LMS);
